...
{!! Form::model($planaccion, ['route' => ['planaccionhallazgos.update', $planaccion->id_planaccionhallazgos], 'method' => 'PUT', 'files' =>true ]) !!}
...
<div style="display: flex; align-items: center;">
        <span>
        {{ Breadcrumbs::render('editarplanaccionhallazgos', $id_auditoriaprog) }}	
        </span>
...
<div class="card-body floating-label">
                ...
                {{-- TABS HEADERS --}}
					<h2><b>Editar Plan Acción Hallazgos</b></h2>
					<hr>
					<br><br>
                ...
                <div class="form-group control-width-normal">
                    <div class="input-group date" id="demo-date-format">
                        <div class="input-group-content">
                            {{ Form::text('fecha_planaccion', $planaccion->fecha_planaccion, array('class' => 'form-control', 'required' => '' )) }}
    ...
    <tr>
        ...
        <td style="vertical-align: middle;">
            <textarea type="text" name="causa_raiz[]" class="form-control" style="height:41px;border: none; border-bottom: 2px solid #48548c;resize: vertical; overflow-y: auto;" maxlength="2000" required>{{ $hallazgos->causa_raiz }}</textarea>
        </td>
        <td style="vertical-align: middle;">
            <textarea type="text" name="acciones_mejora[]" class="form-control" style="height:41px;border: none; border-bottom: 2px solid #48548c;resize: vertical; overflow-y: auto;" required>{{ $hallazgos->acciones_mejora }}</textarea>
        </td>
        <td style="vertical-align: middle;">
            <div class="input-group date" id="demo-date-format">
                <div class="input-group-content" style="display: flex; align-items: center;">
                    {{ Form::text('fecha_inicio[]', $hallazgos->fecha_inicio, ['class' => 'form-control', 'required' => '', 'style' => 'width:100px;border: none; border-bottom: 2px solid #48548c;' ]) }}
                </div>
                <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
            </div>    
        </td>
        <td style="vertical-align: middle;">
            <div class="input-group date" id="demo-date-format">
                <div class="input-group-content" style="display: flex; align-items: center;">
                    {{ Form::text('fecha_fin[]', $hallazgos->fecha_fin, ['class' => 'form-control', 'required' => '', 'style' => 'width:100px;border: none; border-bottom: 2px solid #48548c;' ]) }}
                </div>
                <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
            </div>    
        </td>
        <td style="vertical-align: middle;">
            <textarea type="text" name="evaluacion[]" class="form-control" style="height:41px; border: none; border-bottom: 2px solid #48548c; resize: vertical; overflow-y: auto;" required>{{ $hallazgos->evaluacion }}</textarea>
        </td>
    </tr>
@endforeach

@foreach($planinformehallazgos as $index => $informeh)
    {{-- Verifica si el tipo_hallazgo y descripcion no están en planaccionhallazgoslista --}}
    @php
        $existsInPlanAccion = false;
        foreach ($planaccionhallazgoslista as $hallazgo) {
            if ($hallazgo->TipoHallazgo == $informeh->TipoHallazgo && $hallazgo->descripcion_hallazgos == $informeh->descripcion) {
                $existsInPlanAccion = true;
                break;
            }
        }
    @endphp

    {{-- Si no existe en planaccion, lo mostramos --}}
    @if(!$existsInPlanAccion)
        <tr>
            <td style="vertical-align: middle;">
                {{ $index + 1 }}
            </td>
            <td style="vertical-align: middle;">
                ...
                {{ Form::text('tipo_hallazgo[]', $informeh->TipoHallazgo, ['class' => 'form-control tipohallazgo_id_listas', 'required' => '', 'style' => 'border: none; border-bottom: 2px solid #48548c;', 'onchange' => 'updateTotals()', 'readonly' => 'readonly']) }}   
            </td>
            <td style="vertical-align: middle;">
        ...
        @enderror
            ...
            <td style="vertical-align: middle;">
            <textarea name="evaluacion[]" class="form-control" style="height:41px; border: none; border-bottom: 2px solid #48548c; resize: vertical; overflow-y: auto; width: 100%;resize: vertical; overflow-y: auto;" maxlength="1000" required></textarea>
            @error('evaluacion.*')
            <div class="text-danger">{{ $message }}</div>
        @enderror
            </td>
        ...
        <div class="row" style="margin-top:50px;">
             <div class="col-sm-12">
                <div class="form-group">
                    <textarea class="form-control" id="observaciones" name="observaciones" rows="2" maxlength="500" required> {{$planaccion->observaciones}} </textarea>
